...
def detect_bar_code(img):
    ...
    #框出数字和字母
    draw_res = cv2.rectangle(res,(max_x,max_y+max_h),(max_x+max_w,max_y+max_h+int(0.6*max_h)),(0,255,0),2)
    #cv_show('res',draw_res)
    ...
    number = pytesseract.image_to_string(Image.open('number.jpg'),lang='eng',config='psm 7 digits')
    #print(pytesseract.image_to_string(Image.open('number.jpg'),lang='eng',config='psm 7 digits'))

    return number,number_roi,roi,draw_res

#test
#img = cv2.imread(r"C:\Users\Owen\Pictures\bar_code_test.jpg")
#num,num_roi,roi,draw_res=detect_bar_code(img)
#print(num)
